...
test("iotm_parsing: Diabolic Pizza Cube: detects making and building the correct additionalDisplay", async () => {
  const sampleText =
    "pizza porquoise, old bronzer, leftovers of indeterminate origin, old eyebrow pencil\n" +
...
test("iotm_parsing: God Lobster: supports getting a boon choice adventure", async () => {
  ...
  expect(firstEntry.additionalDisplay).toBe(
...
test("iotm_parsing: Potted Power Plant: parses acquiring a battery", async () => {
  const sampleText =
    "Took choice 1448/1: (secret choice)\n" +
...
test("iotm_parsing: Potted Power Plant: parses using a battery", async () => {
  ...
  const firstEntry = testStore.allEntries[0];
  expect(firstEntry.entryType).toBe(
    ENTRY_TYPE.IOTM.POTTED_POWER_PLANT.USE_BATTERY,
  );
});

test("iotm_parsing: Potted Power Plant: parses combining two batteries", async () => {
  const sampleText =
    "Combine 1 battery (D) + 1 battery (AA)\n" +
  ...
  const firstEntry = testStore.allEntries[0];
  expect(firstEntry.entryType).toBe(
    ENTRY_TYPE.IOTM.POTTED_POWER_PLANT.COMBINE_BATTERIES,
  );
});
...
test("iotm_parsing: Potted Power Plant: parses untinkering two batteries", async () => {
  ...
  const firstEntry = testStore.allEntries[0];
  expect(firstEntry.entryType).toBe(
    ENTRY_TYPE.IOTM.POTTED_POWER_PLANT.UNTINKER,
  );
});
