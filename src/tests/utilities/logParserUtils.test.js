...
test("logParserUtils.parseDifficultyName(): parses the three different difficulty names", () => {
  ...
  expect(logParserUtils.parseDifficultyName(HCMockText)).toBe("Hardcore");

  const SCMockText =
    "Ascension #111:\nNormal Low Key Summer Pastamancer\nWallaby";
  expect(logParserUtils.parseDifficultyName(SCMockText)).toBe("Normal");

  const CASMockText =
    "Ascension #111:\nCasual Low Key Summer Pastamancer\nWallaby";
  expect(logParserUtils.parseDifficultyName(CASMockText)).toBe("Casual");
});
...
test("logParserUtils.parsePathName(): recognizes Ed the Undying path", () => {
  ...
  expect(logParserUtils.parsePathName(ETUMockText)).toBe(
...
test("logParserUtils.createPathLabel(): creates the label as expected", () => {
  ...
  const OXYMockText =
    "Ascension #111:\nHardcore Oxygenarian PastamancerMongoose";
