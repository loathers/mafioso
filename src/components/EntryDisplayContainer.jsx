...
import AdventureSVG from "../images/sands-of-time.svg";
import FamiliarSVG from "../images/cat.svg";
import HealthSVG from "../images/glass-heart.svg";
import ManaSVG from "../images/potion-ball.svg";
import SpellbookSVG from "../images/spell-book.svg";
import LevelUpSVG from "../images/star-formation.svg";
import EditSVG from "../images/scroll-quill.svg";
import SteakSVG from "../images/steak.svg";
import SwapBagSVG from "../images/swap-bag.svg";
import TalkSVG from "../images/talk.svg";

import Button from "./Button";
import CombatSequenceDisplay, {
  CombatResultDisplayHandler,
} from "./CombatSequenceDisplay";
import EntryTypeIcon from "./EntryTypeIcon";
import MakeDiabolicPizzaDisplay from "./MakeDiabolicPizzaDisplay";
import StatChangesDisplay from "./StatChangesDisplay";
...
export default function EntryDisplayContainer(props) {
  ...
  const [currentAnnotations, updateAnnotations] = useState(
    entry.attributes.annotations,
  );
  const shouldShowAnnotations =
    currentAnnotations !== null && currentAnnotations !== undefined;

  const onToggleAnnotations = () => {
    if (shouldShowAnnotations) {
      // aka is currently showing
    ...
    <div className={combineClassnames("flex-col position-relative", className)}>
      ...
      <div
        className={combineClassnames(
          entry.isAnnotationOnly ? "display-none" : "flex-row",
        )}
      >
        <Button
          onClick={() => toggleSelected(!isSelected)}
          className={combineClassnames(
            "borradius-l-2 pad-2 overflow-hidden flex-row flex-auto",
            !isShowRightColumn ? "borradius-r-2" : "",
          )}
        >
          ...
          {isShowRaw && (
            <div className="borradius-1 pad-3 pad-r-8 flex-row flex-auto bg-fourth adjacent-mar-l-4">
              <div
                style={{ flex: "1 1 33%" }}
                className={combineClassnames(
                  "flex-col whitespace-pre-wrap wordbreak-break flex-auto adjacent-mar-l-3",
                )}
              >
                {entry.rawText}
              </div>
              ...
              <div
                style={{ flex: "1 1 67%" }}
                className="pad-2 whitespace-pre-wrap bor-l-1-grayest flex-col flex-auto adjacent-mar-l-3"
              >
        ...
        {isShowRightColumn && (
          <div className="borradius-r-2 bg-second flex-row pad-2 flex-none">
            <Button
              ...
              className={combineClassnames(
                "position-relative flex-row jcontent-center fontsize-1 flex-auto adjacent-mar-l-2",
                shouldShowAnnotations ? "unusable" : "",
              )}
            >
              <img
                src={TalkSVG}
...
function EntryAdventureColumn(props) {
  ...
  const fontClassName =
    !shouldHideTurnNum && entry.isAdventure
      ? "fontsize-5 f-bold color-white"
  ...
  return (
    ...
    >
      <div
        className={combineClassnames(
          "talign-right width-full aself-start adjacent-mar-t-2",
          fontClassName,
        )}
      >
      ...
      {entry.isForcedAdventure && (
        <div className="talign-right fontsize-2 adjacent-mar-t-2">forced</div>
      )}

      {entry.isFreeCombat && (
        <div className="talign-right fontsize-2 adjacent-mar-t-2">free</div>
      )}
    </div>
  );
}
    ...
    <div className={combineClassnames("flex-row adjacent-mar-t-2", className)}>
      ...
      <div className="flex-col flex-auto adjacent-mar-l-3">
        {entry.locationDisplay && (
          <div className="fontsize-2 color-gray flex-none adjacent-mar-t-1">
            {entry.locationDisplay}
          </div>
        )}
        ...
        <div className="fontsize-5 overflow-hidden flex-row flex-none adjacent-mar-t-1">
          ...
          {entry.showAdditionalDisplay && entry.hasAdditionalDisplay && (
            <div className="flex-none adjacent-mar-l-5">
      ...
      <div className="flex-col flex-none adjacent-mar-l-3">
        ...
        {entry.hasFamiliarUsed && (
          <div className="flex-row flex-none adjacent-mar-t-1">
            ...
            />
            <div className="fontsize-2 color-gray flex-none adjacent-mar-l-2">
              {entry.familiarUsed}
            </div>
          </div>
        )}
...
function EntryBodyContainer(props) {
  ...
  return (
    <div
      className={combineClassnames("flex-col whitespace-pre-wrap", className)}
    >
      {/* text content */}
      ...
      {entry.hasStatChanges && (
        <StatChangesDisplay entry={entry} className="" />
      )}
  ...
  return (
    ...
    >
      <div className="flex-col mar-h-5 adjacent-mar-l-3">
        {!isEditing && (
          <img src={TalkSVG} style={{ width: 20, height: 20, opacity: 0.5 }} />
        )}
        {isEditing && (
          <img src={EditSVG} style={{ width: 20, height: 20, opacity: 0.5 }} />
        )}
      </div>
